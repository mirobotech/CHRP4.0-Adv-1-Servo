/*==============================================================================
 Project: Adv-1-Servo                   Activity: mirobo.tech/chrp4-advanced-1
 Date:    May 24, 2023
 
 This advanced programming activity for the mirobo.tech CHRP4 demonstrates the
...
int main(void)
{
    ...
    CHRP4_config();             // Configure I/O for on-board CHRP4 devices and  
 ...
 *      operators as shown in this code from the servo_pulse() function:
    ...
    LATB = LATB | servo;        // Set servo pin high and start fixed delay
 ...
 *      on all four PORTB pins (all H1-H4 header pins on CHRP4) in sequence,
 ...
 *      TMR0 interrupt is configured and enabled in the CHRP4_config() function,
 ...
 *      following code to the CHRP4_config() function in the CHRP4.c file:

    // Configure PORTB IOC (Interrupt-on-Change) interrupt
    IOCBF = 0;                  // Clear all PORTB IOC flags
    IOCBN = 0b10000000;         // Enable negative-edge IOC for SW5
    IOCIF = 0;                  // Clear IOC interrupt flag
    IOCIE = 1;                  // Enable IOC interrupts

 *      This IOC configuration enables SW5 to generate an interrupt on the
 *      negative-going edge of the switch pin, corresponding to a key press.
 * 
 *      Next, add the following program code to the servo_int() function, after
 *      the existing TMR0 service code, to service the IOC interrupt:
 ...
 *      slowly due to the 50ms long delay in the while loop. Pressing SW5
 *      instantly changes the value of the servo1Position variable through
